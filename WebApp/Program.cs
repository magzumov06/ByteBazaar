...
using Infrastructure.Helpers.CacheHelper;
...
//Redis
builder.Services.AddStackExchangeRedisCache(op => op.Configuration = builder.Configuration.GetConnectionString("RedisCache") );
...
builder.Services.AddScoped<IProductService>(st=> new ProductService(
    st.GetRequiredService<DataContext>(),
    st.GetRequiredService<IFileStorage>(),
    st.GetRequiredService<ICacheService>()
    ));
