...
{
    private string cacheKey = "ProductService";
    ...
    public async Task<Responce<GetProductDto>> GetProductById(int id)
    {
        try
        {
            ...
            var r =  cache.GetAsync<List<Product>>(cacheKey).Result;
            if (r != null)
            {
                var product1 =  r.FirstOrDefault(x => x.Id == id);
                ...
                return new Responce<GetProductDto>(res);
            }
            var product = await context.Products
                .FirstOrDefaultAsync(x => x.Id == id && x.IsDeleted == false );
            await cache.AddAsync(cacheKey, product, DateTime.UtcNow.AddDays(1));
